# Hi, I'm Wendy 👋

🎓 MSc Finance & Management (Distinction)  
📊 Financial Analyst | Data-Driven Decision Maker  
 

---

## 💼 About Me

I am a finance professional with a strong academic background in Economics and Finance, passionate about transforming data into actionable business insights.

I specialise in:
- Financial analysis & reporting
- Budgeting & forecasting
- Financial modelling
- Reward reporting
- Business intelligence & performance tracking

My goal is to support strategic decision-making through accurate financial insights and data visualisation.

---

## 🧠 Core Skills

![Excel](https://img.shields.io/badge/Excel-217346?style=for-the-badge&logo=microsoft-excel&logoColor=white)
![Power BI](https://img.shields.io/badge/Power%20BI-F2C811?style=for-the-badge&logo=powerbi&logoColor=black)
![Financial Analysis](https://img.shields.io/badge/Financial%20Analysis-blue?style=for-the-badge)
![Financial Modelling](https://img.shields.io/badge/Financial%20Modelling-darkblue?style=for-the-badge)
![Budgeting](https://img.shields.io/badge/Budgeting-green?style=for-the-badge)
![Forecasting](https://img.shields.io/badge/Forecasting-orange?style=for-the-badge)
![Business Intelligence](https://img.shields.io/badge/Business%20Intelligence-purple?style=for-the-badge)
![Economics](https://img.shields.io/badge/Economics-grey?style=for-the-badge)

---

## 📊 Featured Projects

### 📌 Excel Financial Dashboard
A dynamic Excel dashboard analysing revenue, expenses, and profit trends over time.

**Key Features:**
- KPI tracking (Revenue, Profit, Margins)
- Monthly trend analysis
- Interactive charts & summaries

---

### 📌 Power BI Sales Performance Dashboard
A business intelligence dashboard built to analyse sales performance across regions and products.

**Key Features:**
- Interactive filters (slicers)
- Regional performance breakdown
- KPI cards (Revenue, Orders, Profit)

---

### 📌 Budget vs Actual Analysis
A financial control model comparing planned budgets against actual performance.

**Key Features:**
- Variance analysis (favourable/unfavourable)
- Expense tracking
- Visual performance reporting

---

## 📈 What I Bring

✔ Strong analytical and problem-solving skills  
✔ Ability to interpret complex financial data  
✔ Proficiency in Excel & Power BI  
✔ Attention to detail and accuracy in reporting  
✔ Business-focused mindset  

---

## 🎯 Career Interests

- Financial Analyst  
- Finance Graduate Roles  
- Business Intelligence Analyst
- Reward Reporting Analyst 
- FP&A (Financial Planning & Analysis)  

---

## 📫 Contact

📧 Email: eyramwen@gmail.com 
🔗 LinkedIn: your LinkedIn here  

---

⭐ *“Turning data into decisions that drive business performance.”*
